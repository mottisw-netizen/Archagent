...
using Archagent.Revit.Commands;

namespace Archagent.Revit
{
    /// <summary>
    /// The protocol, served on loopback.
    ///
    /// Every request is handled on a listener thread, marshalled onto Revit's
    /// thread by <see cref="RevitExecutor"/>, and answered as JSON. The listener
    /// binds to 127.0.0.1 only: this is a local bridge for the architect's own
    /// machine, not a network service.
    /// </summary>
    internal sealed class HostServer
    {
        private readonly RevitExecutor _executor;
        ...
        public HostServer(RevitExecutor executor, int port, string token)
        {
            _executor = executor;
            Port = port;
        ...
        public void Start()
        {
            ...
            _listener.Start();
            _thread = new Thread(Loop) { IsBackground = true, Name = "archagent-host" };
            _thread.Start();
        }
        ...
        private void Handle(HttpListenerContext context)
        {
            ...
            try
            {
                ...
                // Revit's thread runs the work; this thread waits for the answer.
                var result = _executor.Run(app => Route(app, path, request),
            ...
            {
                Respond(context, 400, new Dictionary<string, object>
                    { { "error", Protocol.ErrHost }, { "message", "Revit: " + error.Message } });
            }
        ...
        /// <summary>Runs on Revit's thread, inside an API context.</summary>
        private static object Route(UIApplication app, string path,
            Dictionary<string, object> request)
        {
            Document doc = app.ActiveUIDocument?.Document;
            if (doc == null && path != Protocol.Health)
                throw new HostException(Protocol.ErrHost, "no document is open in Revit");
            ...
            switch (path)
            {
                ...
                case Protocol.Highlight: return HighlightCommand.Run(app, doc, request);
                case Protocol.Export: return ExportCommand.Run(app, doc, request);
                case Protocol.SaveAs: return ExportCommand.SaveAs(doc, request);
                case Protocol.Begin:
                case Protocol.Commit:
                case Protocol.Rollback:
                    // Revit cannot hold a transaction open between requests, so
                    // this host implements the batch form of /apply instead.
                    throw HostException.Unsupported(
                        "this host applies a plan in one /apply call; per-action " +
                        "transactions are not available in Revit");
                default:
